...
def generator(z, y, batch_size=batch_size, phase_train=True, reuse=False):
    ...
    with tf.variable_scope("gen", reuse=reuse):
        ...
        with tf.variable_scope('conv1'):
            g_1 = tf.nn.conv3d_transpose(z, weights['wg1'], (batch_size, 4, 4, 4, 256), strides=[1, 1, 1, 1, 1],
                                         padding="VALID")
            g_1 = tf.nn.bias_add(g_1, biases['bg1'])
            g_1 = tf.contrib.layers.batch_norm(g_1, is_training=phase_train)
            g_1 = tf.nn.relu(g_1)
        with tf.variable_scope('conv2'):
            g_2 = tf.nn.conv3d_transpose(g_1, weights['wg2'], (batch_size, 16, 16, 16, 64),  strides=[1, 4, 4, 4, 1],
                                         padding="SAME")
        ...
        with tf.variable_scope('conv5'):
            g_5 = tf.nn.conv3d_transpose(g_2, weights['wg5'], (batch_size, 64, 64, 64, 1), strides=[1, 4, 4, 4, 1],
                                         padding="SAME")
...
def initialiseWeights():
    ...
    weights['wg1'] = tf.get_variable("wg1", shape=[4, 4, 4, 256, 1024 + y_dim], initializer=xavier_init)
    weights['wg2'] = tf.get_variable("wg2", shape=[4, 4, 4, 64, 256], initializer=xavier_init)
...
def initialiseBiases():
    ...
    biases['bg1'] = tf.get_variable("bg1", shape=[256], initializer=zero_init)
    biases['bg2'] = tf.get_variable("bg2", shape=[64], initializer=zero_init)
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(n_epochs):
            ...
            if epoch % 400 == 10:
                ...
                saver.save(sess, save_path=model_directory + '/Larry_3_2_' + str(epoch) + '.cptk')
