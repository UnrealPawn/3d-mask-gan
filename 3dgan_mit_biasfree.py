...
def initialiseWeights():
    ...
    weights['wcov2'] = tf.get_variable("wcov2", shape=[2, 2, 1, 8], initializer=xavier_init)  # Larry
...
def trainGAN(is_dummy=False, checkpoint=None):
    ...
    para_g = [var for var in tf.trainable_variables() if any(x in var.name for x in ['encoder_h','encoder_b','wg', 'bg', 'gen'])]
    para_d = [var for var in tf.trainable_variables() if any(x in var.name for x in ['encoder_h','encoder_b','wd', 'bd', 'dis'])]
    ...
    with tf.variable_scope(tf.get_variable_scope(), reuse=None):
        ...
        with tf.Session(config=config) as sess:
            ...
            while True:
                ...
                x = np.array(x)
                z_sample= np.array(x)
                z_sample =z_sample.reshape([-1,z_size])
