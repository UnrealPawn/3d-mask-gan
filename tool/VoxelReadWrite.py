...
def importVoxByPath(path,size):
    ...
    for i in os.walk(path):
        for filename in i[2]:
            ...
            # result_array=np.append(result_array,file_array_fliped)
            count=count+1
    result_array=result_array.reshape((count,size,size,size))
    np.save('deep256.db',result_array)
    return result_array
...
if __name__=='__main__':
    ma=importVoxByPath('rD:\RockGan/voxmodel_2_64',64)
    #ma = loadimage(r'C:\Users\Administrator\Desktop\image256\image_6\images\00033500-168-outputs.png',128)
    showVox(ma)
    #showVox(ma[0])
    #showVox(ma[1])
    #showVox(ma[2])
