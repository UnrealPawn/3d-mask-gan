#!/usr/bin/env python
# use vox_fill2 and  set Adam lr and beta to defualt value
...
model_directory = './models/Larry_4/'
...
def generator(z, y, batch_size=batch_size, phase_train=True, reuse=False):
    ...
    with tf.variable_scope("gen", reuse=reuse):
        ...
        g_00 = tf.nn.conv2d(z,weights['w1'],strides=[1, 2, 2, 1],padding="SAME")
        g_00 = tf.contrib.layers.batch_norm(g_00, is_training=phase_train)
        g_00 = tf.nn.leaky_relu(g_00)

        g_01 = tf.nn.conv2d(g_00, weights['w2'], strides=[1, 1, 1, 1], padding="SAME")
        g_01 = tf.contrib.layers.batch_norm(g_01, is_training=phase_train)
        g_01 = tf.nn.leaky_relu(g_01)

        g_02 = tf.nn.conv2d(g_01, weights['w3'], strides=[1, 1, 1, 1], padding="SAME")
        g_02 = tf.contrib.layers.batch_norm(g_02, is_training=phase_train)
        g_02 = tf.nn.leaky_relu(g_02)

        g_03 = tf.nn.conv2d(g_02, weights['w4'], strides=[1, 1, 1, 1], padding="SAME")
        g_03 = tf.contrib.layers.batch_norm(g_03, is_training=phase_train)
        #g_03 = tf.concat([g_03 ,g_01], 3)
        g_03 = tf.nn.leaky_relu(g_03)
        g_03 =  tf.reshape(g_03, shape=[batch_size, 64, 64, 64, 8])

        g_04 = tf.nn.conv3d(g_03, weights['w6'], strides=[1,1, 1, 1, 1], padding="SAME")
        g_04 = tf.contrib.layers.batch_norm(g_04, is_training=phase_train)
        g_04 = tf.nn.leaky_relu(g_04)

        g_05 = tf.nn.conv3d(g_04, weights['w7'], strides=[1,1, 1, 1, 1], padding="SAME")
        g_05 = tf.contrib.layers.batch_norm(g_05, is_training=phase_train)

        g_05 = tf.nn.sigmoid(g_05)
        #g_03 = tf.nn.sigmoid(g_03)
    return g_05
        ...
        #                                 padding='SAME',
        #                                 weights_initializer=tf.random_normal_initializer(0, 0.02))
        #f0 = tf.contrib.layers.flatten(g_00)
        #f1 = tf.contrib.layers.flatten(g_01)
        # f2 = tf.contrib.layers.flatten(g_02)
    #     f3 = tf.contrib.layers.flatten(g_03)
    #     #f_all = tf.concat([f1 ,f3], 1)
    #     z = tf.contrib.layers.fully_connected(f3, 1024, activation_fn=lrelu,
...
def initialiseWeights():
    ...
    weights['w1'] = tf.get_variable("w1", shape=[3, 3, 3, 16], initializer=xavier_init)
    weights['w2'] = tf.get_variable("w2", shape=[3, 3, 16, 64], initializer=xavier_init)
    weights['w3'] = tf.get_variable("w3", shape=[3, 3, 64, 256], initializer=xavier_init)
    weights['w4'] = tf.get_variable("w4", shape=[3, 3, 256, 512], initializer=xavier_init)
    weights['w5'] = tf.get_variable("w5", shape=[3, 3, 512, 1024], initializer=xavier_init)

    weights['w6'] = tf.get_variable("w6", shape=[4, 4, 4, 8, 4], initializer=xavier_init)
    weights['w7'] = tf.get_variable("w7", shape=[4, 4, 4, 4, 1], initializer=xavier_init)
...
def trainGAN(is_dummy=False, checkpoint=None):
    ...
    para_g = [var for var in tf.trainable_variables() if
              any(x in var.name for x in ['encoder_h', 'encoder_b', 'wg', 'bg', 'gen','w'])]
    para_d = [var for var in tf.trainable_variables() if
              any(x in var.name for x in ['encoder_h', 'encoder_b', 'wd', 'bd', 'dis','w'])]
        ...
        for epoch in range(n_epochs):
            ...
            summary_d, discriminator_loss = sess.run([d_summary_merge, d_loss],
                                                     feed_dict={z_vector: z, x_vector: x, y_vector: y,
                                                                y_mask: y_mask_array})
            summary_g, generator_loss,generator_loss_L1 = sess.run([summary_g_loss, g_loss,gen_loss_L1],
                                                 feed_dict={z_vector: z, x_vector: x, y_vector: y,
                                                            y_mask: y_mask_array})
            ...
            if epoch % 400 == 10:
                ...
                saver.save(sess, save_path=model_directory + '/Larry_4_2_' + str(epoch) + '.cptk')
                loss_all = 0
                count = 0
