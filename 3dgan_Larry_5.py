...
model_directory = './models/Larry_5'
npypath = r'./Image_64_3_all.npy'
npypath1 = r'./Image_new.db.npy'
is_local = False
os.environ["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"
os.environ["CUDA_VISIBLE_DEVICES"] = "0"

# config = tf.ConfigProto(
#     device_count={'GPU': 0}0.0393531944327 0.932809072017 0.0373110558941
# )
weights , biases = {}, {}
mask_load = np.load(r"./mask_new.db.npy")
mask_load  =mask_load.reshape([-1,64,64,64,1])

z_load = np.load(npypath) - (0.0393531944327, 0.932809072017 ,0.0373110558941)

z_load1 = np.load(npypath1)
z_load1 = z_load1.reshape([-1,64,64,1]) -  0.228726445412
...
def generator(z, y, batch_size=batch_size, phase_train=True, reuse=False):
    ...
    with tf.variable_scope("gen", reuse=reuse):
        ...
        g_01 = tf.contrib.layers.conv2d(g_00, num_outputs=64, kernel_size=3,
                                        ...
                                        padding='SAME',
                                        weights_initializer=tf.contrib.layers.xavier_initializer())
        g_02 = tf.contrib.layers.conv2d(g_01, num_outputs=128, kernel_size=3,
                                        stride=2, activation_fn=lrelu, normalizer_fn=tf.contrib.layers.batch_norm,
                                        padding='SAME',
                                        weights_initializer=tf.contrib.layers.xavier_initializer())
        g_03 = tf.contrib.layers.conv2d(g_02, num_outputs=256, kernel_size=3,
                                        stride=2, activation_fn=lrelu, normalizer_fn=tf.contrib.layers.batch_norm,
                                        padding='SAME',
                                        weights_initializer=tf.contrib.layers.xavier_initializer())
        g_04 = tf.contrib.layers.conv2d(g_03, num_outputs=512, kernel_size=3,
                                        stride=2, activation_fn=lrelu, normalizer_fn=tf.contrib.layers.batch_norm,
                                        padding='SAME',
                                        weights_initializer=tf.contrib.layers.xavier_initializer())
        z = tf.contrib.layers.conv2d(g_04, num_outputs=1024, kernel_size=3,
                                        stride=2, activation_fn=lrelu, normalizer_fn=tf.contrib.layers.batch_norm,
                                        padding='SAME',
                                        weights_initializer=tf.contrib.layers.xavier_initializer())
        #z = tf.contrib.layers.fully_connected(tf.contrib.layers.flatten(g_02), 1024, activation_fn=lrelu,
...
def initialiseWeights():
    ...
    weights['wg1'] = tf.get_variable("wg1", shape=[4, 4, 4, 512, 1024], initializer=xavier_init)
...
def trainGAN(is_dummy=False, checkpoint=None):
    ...
    g_loss = tf.reduce_mean(g_loss)#*0.05 + tf.cast(1 - loss_L2, tf.float32)*0.95
        ...
        for epoch in range(n_epochs):
            ...
            summary_d, discriminator_loss = sess.run([d_summary_merge, d_loss],
                                                     feed_dict={z_vector: z, x_vector: x, y_vector: y,
                                                                y_mask: y_mask_array})
            summary_g, generator_loss ,L2= sess.run([summary_g_loss, g_loss ,loss_L2 ],
                                                 feed_dict={z_vector: z, x_vector: x, y_vector: y,
                                                            y_mask: y_mask_array})
            ...
            if epoch % 400 == 10:
                ...
                saver.save(sess, save_path=model_directory + '/Larry_5_1_' + str(epoch) + '.cptk')
...
def ganGetOneResult1(trained_model_path):
    global IsInit
    global Istrue
    global number

    if IsInit == False:
        initialiseWeights(),initialiseBiases()
        #initialiseBiases()
        IsInit = False
            ...
            while True:
                if 0:
                   ...
                   #             one_array[i][j] = 0
                    ...
                    x = x - (0.0393531944327, 0.932809072017 ,0.0373110558941)
...
def ganGetOneResult2(trained_model_path):
    global IsInit
    global Istrue
    global number
            ...
            while True:
                if Istrue:
                   ...
                   #
                    ...
                    x = np.array(x)/255
                    z_sample = np.array(x)
                    z_sample = z_sample.reshape([-1, z_size, z_size, 1]) -   0.228726445412
                    y = np.random.normal(0, 0.33, size=[batch_size, y_dim]).astype(np.float32)
                    g_objects = sess.run(net_g_test, feed_dict={z_vector: z_sample, y_vector: y})
                    id_ch = np.random.randint(0, batch_size, 4)
                    i = 0
