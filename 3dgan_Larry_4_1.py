...
def generator(z, y, batch_size=batch_size, phase_train=True, reuse=False):
    ...
    with tf.variable_scope("gen", reuse=reuse):
        ...
        g_01 = tf.nn.conv2d(g_00, weights['w2'], strides=[1, 2, 2, 1], padding="SAME")
        g_01 = tf.contrib.layers.batch_norm(g_01, is_training=phase_train)
        g_01 = tf.nn.leaky_relu(g_01)

        g_02 = tf.nn.conv2d(g_01, weights['w3'], strides=[1, 2, 2, 1], padding="SAME")
        g_02 = tf.contrib.layers.batch_norm(g_02, is_training=phase_train)
        g_02 = tf.nn.leaky_relu(g_02)

        g_03 = tf.nn.conv2d(g_02, weights['w4'], strides=[1, 2, 2, 1], padding="SAME")
...
def initialiseWeights():
    ...
    weights['w1'] = tf.get_variable("w1", shape=[3, 3, 3, 8], initializer=xavier_init)
    weights['w2'] = tf.get_variable("w2", shape=[3, 3, 8, 16], initializer=xavier_init)
    weights['w3'] = tf.get_variable("w3", shape=[3, 3, 16, 32], initializer=xavier_init)
    weights['w4'] = tf.get_variable("w4", shape=[3, 3, 32, 64], initializer=xavier_init)
    weights['w5'] = tf.get_variable("w5", shape=[3, 3, 3, 16], initializer=xavier_init)
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(n_epochs):
            ...
            if epoch % 400 == 10:
                ...
                saver.save(sess, save_path=model_directory + '/Larry_4_1_' + str(epoch) + '.cptk')
                loss_all = 0
                count = 0
