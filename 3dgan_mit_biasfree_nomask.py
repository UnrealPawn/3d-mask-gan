...
npypath = r'D:\RockGan\3DGANServer\tool\X.npy'
npypathy = r'D:\RockGan\3DGANServer\tool\Y.npy'
is_local = False
...
z_load = z_load.reshape([-1,32,32,1])
       ...
       # test_mask_list=d.get_mask_list_by_bound(test_bound_list)
        for epoch in range(n_epochs):
           ...
           # y = bound_list[idx]/64
            #z = z_load[idx]
            y = y_load[idx]
            #y_mask_array = mask_list[idx]#Larry
            # Larry Begin
            ...
            summary_g, generator_loss = sess.run([summary_g_loss, g_loss], feed_dict={z_vector: z, y_vector: y})
            d_accuracy, n_x, n_z = sess.run([d_acc, n_p_x, n_p_z],
                                            feed_dict={z_vector: z, x_vector: x,y_vector: y})
            print(n_x, n_z)

            if d_accuracy < d_thresh:
                sess.run([optimizer_op_d], feed_dict={z_vector: z,x_vector: x,y_vector:y})
            ...
            sess.run([optimizer_op_g], feed_dict={z_vector: z,y_vector:y})
            ...
            if epoch % 200 == 10:
                ...
                saver.save(sess, save_path=model_directory + '/Larry_3_' + str(epoch) + '.cptk')
            # if epoch == 10:
    ...
    with tf.variable_scope(tf.get_variable_scope(), reuse=None):
        ...
        with tf.Session(config=config) as sess:
            ...
            while True:
                ...
                num = np.random.randint(len(y_load),size = 1)
                img_np_1d = y_load[num]
                print(num)
                for i in range(0,5):
                    x.append(img_np_1d)
                x = np.array(x)
