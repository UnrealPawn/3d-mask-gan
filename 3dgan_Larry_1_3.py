...
def trainGAN(is_dummy=False, checkpoint=None):
    ...
    loss_L2 = tf.reduce_sum(tf.cast(tf.logical_and(a, b), tf.int32)) / tf.reduce_sum(
        tf.cast(tf.logical_or(a, b), tf.int32))
    d_loss = tf.reduce_mean(d_loss)
    gen_loss_L1 = tf.reduce_mean(tf.abs(x_vector - net_g_train))
    g_loss = tf.reduce_mean(g_loss)*0.1 + gen_loss_L1*0.9 + tf.cast(1 - loss_L2, tf.float32)
        ...
        for epoch in range(n_epochs):
            ...
            summary_d, discriminator_loss = sess.run([d_summary_merge, d_loss],
                                                     feed_dict={z_vector: z, x_vector: x, y_vector: y,
                                                                y_mask: y_mask_array})
            summary_g, generator_loss , generator_loss_L1 , L2= sess.run([summary_g_loss, g_loss,gen_loss_L1 , loss_L2],
                                                 feed_dict={z_vector: z, x_vector: x, y_vector: y,
                                                            y_mask: y_mask_array})
            ...
            if epoch % 400 == 10:
                ...
                saver.save(sess, save_path=model_directory + '/Larry_1_3_' + str(epoch) + '.cptk')
