...
model_directory = './models/Larry_3/'
...
def generator(z, y, batch_size=batch_size, phase_train=True, reuse=False):
    ...
    with tf.variable_scope("gen", reuse=reuse):
        ...
        z = tf.contrib.layers.fully_connected(tf.contrib.layers.flatten(g_03), 512, activation_fn=lrelu,
                                              weights_initializer=tf.random_normal_initializer(0, 0.02))

        z = tf.reshape(z, (batch_size, 1, 1, 1, 512))
        ...
        with tf.variable_scope('conv2'):
            g_2 = tf.nn.conv3d_transpose(g_1, weights['wg2'], (batch_size, 8, 8, 8, 256), strides=[1, 2, 2, 2, 1],
                                         padding="SAME")
        ...
        with tf.variable_scope('conv4'):
            g_4 = tf.nn.conv3d_transpose(g_3, weights['wg4'], (batch_size, 32, 32, 32, 64), strides=strides,
                                         padding="SAME")
            g_4 = tf.nn.bias_add(g_4, biases['bg4'])
            g_4 = tf.contrib.layers.batch_norm(g_4, is_training=phase_train)
            g_4 = tf.nn.relu(g_4)
        with tf.variable_scope('conv5'):
            g_5 = tf.nn.conv3d_transpose(g_4, weights['wg5'], (batch_size, 64, 64, 64, 1), strides=strides,
                                         padding="SAME")
...
def initialiseWeights():
    ...
    weights['wg1'] = tf.get_variable("wg1", shape=[4, 4, 4, 512, 512 + y_dim], initializer=xavier_init)
    weights['wg2'] = tf.get_variable("wg2", shape=[3, 3, 3, 256, 512], initializer=xavier_init)
    weights['wg3'] = tf.get_variable("wg3", shape=[3, 3, 3, 128, 256], initializer=xavier_init)
    weights['wg4'] = tf.get_variable("wg4", shape=[3, 3, 3, 64, 128], initializer=xavier_init)
    weights['wg5'] = tf.get_variable("wg5", shape=[3, 3, 3, 1, 64], initializer=xavier_init)
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(n_epochs):
            ...
            if epoch % 400 == 10:
                ...
                saver.save(sess, save_path=model_directory + '/Larry_3_1_2_' + str(epoch) + '.cptk')
                loss_all = 0
                count = 0
